...
export function TechniciansAdminPanel({ initialTechnicians }: { initialTechnicians: TechnicianItem[] }) {
    ...
    return (
        <section className="space-y-4 rounded-md border border-[#e5e7eb] bg-white p-4">
            <div>
                <h2 className="text-sm font-semibold">Technician Rates and Availability</h2>
                <p className="mt-1 text-xs text-[#475569]">Admins can manage labor rates and current availability for assignment during job intake.</p>
            </div>

            <form className="grid gap-2 rounded-md border border-[#e2e8f0] bg-[#f8fafc] p-3 sm:grid-cols-2 lg:grid-cols-5" onSubmit={submitCreate}>
