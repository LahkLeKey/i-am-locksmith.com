...
describe('geocodeAddress', () => {
  ...
  it('returns a fresh database cache hit without calling Nominatim',
     async () => {
       ...
       const result = await geocodeAddress(
           {query: '  123 Main St  ', limit: 5, language: 'en'});
  ...
  it('stores a normalized provider response after a cache miss', async () => {
    ...
    const result =
        await geocodeAddress({query: '123 Main St', limit: 5, language: 'en'});
  ...
  it('serves stale data when another invocation owns the refresh lease',
     async () => {
       mockedGetGeoCache.mockResolvedValue({
         status: 'STALE',
         value: [{
  ...
  it('does not call the provider when another invocation owns a cold miss',
     async () => {
       ...
       await expect(
           geocodeAddress({query: '123 Main St', limit: 5, language: 'en'}))
