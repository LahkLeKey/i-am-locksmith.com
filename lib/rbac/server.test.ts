...
import {computeEffectivePermissionsForIdentity, extractScopedUserRole, parseClerkOrgRole,} from './server';
...
describe('extractScopedUserRole', () => {
  it('returns null when org context is missing', () => {
    const role =
        extractScopedUserRole({orgRoles: {org_123: 'dispatcher'}}, null);
  ...
  it('reads role from org-scoped metadata', () => {
    const role = extractScopedUserRole(
        {orgRoles: {org_123: 'inventory_manager'}}, 'org_123');
  ...
  it('ignores unknown org-scoped values', () => {
    const role =
        extractScopedUserRole({orgRoles: {org_123: 'superuser'}}, 'org_123');
