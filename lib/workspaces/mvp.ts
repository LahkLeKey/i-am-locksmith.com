...
export type WorkspaceMvpSnapshot = {
  ...
  queue: Array<{
    title: string;
    detail: string;
    status: 'urgent' | 'attention' | 'scheduled';
  }>;
  checklist: string[];
};
...
const WORKSPACE_MVP_SNAPSHOTS: Record<MvpWorkspaceKey, WorkspaceMvpSnapshot> = {
  customers: {
    ...
    subtitle: 'Track customer records, service history, and outreach tasks tied to active field work.',
    kpis: [
      {label: 'Active customers', value: '182', trend: '+7 this week'},
      {label: 'Service agreements', value: '64', trend: '91% renewed on time'},
      {label: 'At-risk accounts', value: '11', trend: '4 require callback today'},
    ],
    queue: [
      {
  ...
  jobs: {
    ...
    subtitle: 'Move work from quote-approved to dispatched and completed with inventory context.',
    kpis: [
      {label: 'Open jobs', value: '37', trend: '9 assigned this morning'},
      {label: 'On-time completion', value: '94%', trend: '+3% vs last week'},
      {label: 'Blocked by stock', value: '5', trend: '2 escalated to inventory'},
    ],
    queue: [
      {
  ...
  quotes: {
    ...
    subtitle: 'Prepare scoped quotes with labor, parts, and approval timing at a glance.',
    kpis: [
  ...
  invoices: {
    ...
    subtitle: 'Close completed jobs with accurate billing, status tracking, and follow-through.',
    kpis: [
      {label: 'Open invoices', value: '41', trend: '12 due this week'},
      {label: 'Collected this month', value: '$84,200', trend: '+$9,400 vs last month'},
  ...
  reports: {
    ...
    subtitle: 'Review operational trends, profitability, and service outcomes from persisted events.',
    kpis: [
      {label: 'Weekly gross margin', value: '42.1%', trend: '+1.8% week over week'},
  ...
  settings: {
    ...
    subtitle: 'Manage access controls, workflow defaults, and operational guardrails for your org.',
    kpis: [
      {label: 'Active users', value: '29', trend: '3 pending invites'},
      {label: 'Role mappings', value: '6 profiles', trend: '2 updated this month'},
...
export function getWorkspaceMvpSnapshot(
    workspaceKey: MvpWorkspaceKey): WorkspaceMvpSnapshot {
  return WORKSPACE_MVP_SNAPSHOTS[workspaceKey];
}
