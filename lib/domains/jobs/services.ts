...
export class JobService {
   ...
   */
  static getNextRecommendedStatus(currentStatus: JobQueueStatus):
      JobQueueStatus {
...
export class JobWorkflowService {
  ...
  static canBeQuoted(draft: JobDraft): boolean {
    return !!(
        draft.customerName?.trim() && draft.site?.trim() &&
  ...
  static canBeScheduled(draft: JobDraft): boolean {
    return !!(
        draft.quotePartEstimate && Number(draft.quotePartEstimate) > 0 &&
        draft.scheduledFor && draft.assignedTechnicianIds?.length);
  }
  ...
  static validateWorkflowStep(
    ...
    switch (step) {
      ...
      case 2:  // Schedule and parts
        ...
        if (draft.requiredSkus?.length === 0)
          errors.push('Select at least one part');
        break;

      case 3:  // Technician and estimates
        if (!draft.assignedTechnicianIds?.length)
          errors.push('Technician assignment required');
        if (!draft.estimatedMinutes || Number(draft.estimatedMinutes) <= 0) {
