 ...
 * a single assigned technician. Components handle the mapping between form
 * state and persisted data through conversion during load/save cycles.
 *
 * This is the canonical form state type used across all job-related components
 * and wizards. Keep this type in sync with the actual form fields in
 * components.
 */
...
export type JobQuote = {
  jobId: string; partEstimate: number; laborEstimate: number; notes: string;
  createdAt: string;
  updatedAt: string;
};
...
export type TimeClockEntry = {
  id: string; jobId: string; technicianId: string;
...
export type TimeClockTableActionDraft = {
  entryId: string; mode: TimeClockTableActionMode; value: string;
};
...
export type LedgerPairAnalysis = {
  entries: LedgerAnnotatedEntry[]; pairedCount: number; openCount: number;
...
export type LedgerPairRow = {
  id: string; clockIn: TimeClockEntry | null; clockOut: TimeClockEntry | null;
...
export type CloseoutDraft = {
  actualPartCost: string; actualLaborCost: string; actualMinutes: string;
  finalTotal: string;
...
export type JobCloseout = {
  jobId: string; closureStatus: string; actualPartsCost: number;
