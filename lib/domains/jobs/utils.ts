 ...
 */
export function computePartEstimateFromSkus(requiredSkus: string[], parts: InventoryPart[]): number {
 ...
 */
export function upsertJob(current: JobQueueItem[], nextJob: JobQueueItem): JobQueueItem[] {
 ...
 */
export function ensureLedgerPairs(ledger: EditableLedgerEntry[]): EditableLedgerEntry[] {
    if (ledger.length === 0) {
        return [{
            id: 'temp-0',
            jobId: '',
 ...
 */
export function validateLedgerPairs(ledger: EditableLedgerEntry[]): LedgerPairValidation {
    const errors: string[] = [];
    ...
    for (let i = 0; i < ledger.length; i++) {
        ...
        if (entry.type === 'clock_out') {
            const hasPrecedingClockIn = ledger.slice(0, i).some((e) => e.type === 'clock_in' && !e.editNote);
 ...
 */
export function determinePairStatus(entry: EditableLedgerEntry, ledger: EditableLedgerEntry[]): LedgerPairStatus {
    const entryIndex = ledger.indexOf(entry);
    
    if (entry.type === 'clock_in') {
        // Look for following clock out
        const followingClockOut = ledger
            .slice(entryIndex + 1)
 ...
 */
export function analyzeLedgerPairs(ledger: EditableLedgerEntry[]): LedgerPairAnalysis {
    const entries: LedgerAnnotatedEntry[] = ledger.map((entry) => ({
        ...entry,
        pairStatus: determinePairStatus(entry, ledger),
    }));
    
    const pairedCount = entries.filter((e) => e.pairStatus === 'paired').length;
    const openCount = entries.filter((e) => e.pairStatus === 'open_clock_in').length;
    const unmatchedCount = entries.filter((e) => e.pairStatus === 'unmatched_clock_out').length;
